...
api = Bootstrap.blueprint(__name__, '/users/permissions')


@api.route('/', methods=['POST'])
def create():
    return response_create(UserPermission)


@api.route('/', methods=['GET'])
def read_many():
    return response_list(UserPermission)


@api.route('/<int:seq>', methods=['GET'])
def read(seq):
    return response_read(UserPermission, seq)


@api.route('/<int:seq>', methods=['PATCH'])
def update(seq):
    return response_update(UserPermission, seq)


@api.route('/', methods=['DELETE'])
@api.route('/<seq>', methods=['DELETE'])
def delete(seq=None):
    return response_delete_any(UserPermission, seq)
...
def functional_path(plural_column_name: str, method_name: str, method_value: str = None):
    return response_functional_path(UserPermission, plural_column_name, method_name, method_value)
